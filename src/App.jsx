...
const Service = {
    ...
    if (action === 'add') {
      ...
      list =
        tripId && type === 'itinerary'
          ? [...list, newItem].sort((a, b) =>
              (a.time || '').localeCompare(b.time || '')
            )
          : [newItem, ...list];
    } else if (action === 'update') {
      list = list.map((i) => (i.id === id ? { ...i, ...data } : i));
      if (tripId && type === 'itinerary')
  ...
  batchSwap: async (tripId, itemA, itemB) => {
    ...
    if (Service.mode === 'cloud' && Service.db) {
      ...
      batch.update(doc(Service.db, ...pathBase, itemA.id), {
        time: itemB.time,
      });
      batch.update(doc(Service.db, ...pathBase, itemB.id), {
        time: itemA.time,
      });
      await batch.commit();
      return null;
      ...
      return await Service.op(
        tripId,
        'itinerary',
        'update',
        { ...itemB, time: itemA.time },
        itemB.id
      );
    }
  },
...
const SwipeableRow = ({
  ...
  return (
    <div
      className={`relative w-full rounded-xl h-auto select-none overflow-visible group touch-pan-y ${className}`}
    >
      ...
      <div
        className="relative z-10 bg-white rounded-xl shadow-sm border border-slate-100 transition-transform duration-200 ease-out h-full"
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/50 flex items-center justify-center p-4 animate-in fade-in">
      <div className="bg-white rounded-xl w-full max-w-sm p-5 flex flex-col">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-bold flex items-center gap-2">
            <Icons.Settings className="text-sky-600" size={20} /> 旅行設定
          </h3>
          <button onClick={onClose}>
            <Icons.X className="text-slate-400" />
          </button>
        ...
        <button
          ...
          className="w-full py-3 bg-sky-600 text-white rounded-lg font-bold text-sm shadow-md"
        >
          儲存變更
        </button>
      </div>
...
function TripList({ trips, onAdd, onDelete, onSelect, mode }) {
  ...
  return (
    <div className="pb-20">
      ...
      />
      <header
        className={`text-white p-6 pt-10 shadow-md rounded-b-3xl mb-6 ${
          mode === 'cloud' ? 'bg-sky-600' : 'bg-slate-600'
        }`}
      >
        ...
        <div className="text-[10px] opacity-80 mt-1 flex items-center gap-1">
          {mode === 'cloud' ? (
            <>
              <Icons.Cloud size={10} /> 家庭共享模式
            </>
          ) : (
            <>
              <Icons.CloudOff size={10} /> 本機試用模式
            </>
          )}
        </div>
      </header>
      ...
      <div className="px-4 space-y-4">
        {!isCreating ? (
          <button
            ...
            className="w-full py-4 border-2 border-dashed border-sky-200 rounded-2xl flex items-center justify-center gap-2 text-sky-600 font-bold hover:bg-sky-50 bg-white"
          >
            <Icons.Plus /> 建立新計畫
          </button>
        ) : (
          <div className="bg-white p-4 rounded-xl shadow-lg border border-sky-100 animate-in fade-in">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="flex-1 bg-sky-600 text-white py-2 rounded text-sm"
              >
                建立
              </button>
            </div>
          ...
          {trips.map((t) => (
            ...
            >
              {t.coverImage ? (
                ...
                <div className="absolute inset-0 bg-gradient-to-r from-sky-50 to-white"></div>
              )}
...
function TripDetail({ trip, mode, onUpdate, onBack }) {
  ...
  useEffect(() => {
    const u1 = Service.subscribe(trip.id, 'itinerary', (d) =>
      setItems(d || [])
    );
    const u2 = Service.subscribe(trip.id, 'memories', (d) =>
      setMemories(d || [])
    );
    return () => {
      if (u1) u1();
  ...
  const handleItemAction = async (type, action, data, id) => {
    ...
    if (Service.mode === 'local' && res)
      type === 'itinerary' ? setItems(res) : setMemories(res);
    setEditOpen(false);
    setEditingItem(null);
  };
  ...
  const handleImport = async (text) => {
    ...
    for (const l of lines) {
      ...
      if (lt.includes('食') || lt === 'food') type = 'food';
        ...
        lt.includes('購') ||
        lt.includes('shopping') ||
        lt.includes('buy') ||
        lt.includes('outlet')
      )
        type = 'shopping';
      else if (lt.includes('通') || lt.includes('transport'))
        type = 'transport';
      else if (lt.includes('住') || lt.includes('stay')) type = 'stay';
      await handleItemAction('itinerary', 'add', {
        day,
        time,
        activity,
        location,
  ...
  return (
    <>
      <ConfirmModal
        ...
        message={
          deleteModal.type === 'batch_day'
            ? `確定要刪除 Day ${day} 的所有行程嗎？`
            : '確定要刪除這個項目嗎？'
        }
        onConfirm={() => {
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={() => {
                if (editingItem)
                  handleItemAction(
                    editingItem.activity ? 'itinerary' : 'memories',
                    'update',
                    editingItem,
                    editingItem.id
                  );
                  ...
                  if (activeTab === 'plan')
                    handleItemAction('itinerary', 'add', {
                      day,
                      ...newItem,
                      completed: false,
                    });
                    ...
                    handleItemAction('memories', 'add', n);
                    setEditOpen(false);
                    setNewMem({
                      text: '',
                      mood: 'happy',
              ...
              className="flex-1 bg-sky-600 text-white py-2 rounded font-bold"
            >
              儲存
            </button>
          </div>
      ...
      <header
        className={`relative text-white p-4 pt-8 shadow-md z-20 ${
          trip.coverImage ? 'h-40' : 'bg-sky-600'
        }`}
      >
        {trip.coverImage && (
          <>
        ...
        <div className="relative z-10 h-full flex flex-col justify-between">
          <div className="flex items-center gap-3">
            ...
            >
              <Icons.Plane className="transform rotate-180" />
            </button>
            <div className="flex-1 min-w-0">
            ...
            {Array.from({ length: totalDays }).map((_, i) => (
              <button
                ...
                className={`flex-shrink-0 w-12 h-14 rounded-xl flex flex-col items-center justify-center text-sm font-medium transition-all ${
                  day === i + 1
                    ? 'bg-white text-sky-600 scale-105 shadow'
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex gap-2">
                {dailyItems.length > 0 && (
                  <button
                    onClick={() =>
                      setDeleteModal({ isOpen: true, type: 'batch_day' })
                    }
                    className="text-red-500 bg-white border border-red-100 p-2 rounded-full shadow-sm"
                  >
                    <Icons.Trash />
                  </button>
                )}
                <button
                  ...
                  className="text-sky-600 bg-white border border-sky-100 p-2 rounded-full"
                >
                  <Icons.FileText />
                </button>
                <button
                  ...
                  className="text-white bg-sky-600 p-2 rounded-full shadow-md"
                >
                  <Icons.Plus />
                </button>
              </div>
                ...
                return (
                  <div key={item.id} className="flex relative">
                    ...
                    <div className="w-14 flex-shrink-0 flex flex-col items-end pr-3 pt-5 relative">
                      <span
                        className={`text-xs font-bold font-mono ${style.text}`}
                      >
                        {item.time}
                      </span>
                    </div>
                    ...
                    <div className="relative flex flex-col items-center w-6 flex-shrink-0">
                      {/* Vertical Line */}
                      <div
                        className={`w-0.5 flex-1 ${style.line} ${
                          isLast
                            ? 'bg-gradient-to-b from-current to-transparent max-h-full'
                            : ''
                        }`}
                      ...
                      {/* Dot */}
                      <div
                        className={`absolute top-5 w-3 h-3 rounded-full border-2 border-white shadow-sm z-10 ${style.dot}`}
                      ></div>
                    </div>
                    ...
                    <div className="flex-1 pb-4 pl-2 min-w-0">
                      ...
                      >
                        <div
                          className={`p-3 rounded-xl border relative shadow-sm transition-all active:scale-[0.98] ${
                            style.bg
                          } ${style.border} ${
                          ...
                          <div className="flex justify-between items-start">
                            <div className="flex-1 min-w-0">
                              ...
                              {(item.notes || safeAtt(item).length > 0) && (
                                <div className="mt-2 bg-white/60 p-2 rounded text-sm text-slate-600 border border-black/5 whitespace-pre-wrap">
            ...
            {dailyMemories.map((m) => {
              ...
              return (
                <SwipeableRow
                  key={m.id}
                  onDeleteRequest={() =>
                    setDeleteModal({ isOpen: true, id: m.id, type: 'memories' })
                  }
                ...
                >
                  <div className="p-3 relative bg-white border border-slate-100 shadow-sm rounded-xl">
                      ...
                      <div className="flex gap-1 mb-2">
                        {safeAtt(m).map((a, i) => (
                          <img
                            key={i}
                            src={a}
                            className="h-20 w-full object-cover rounded bg-slate-100 cursor-pointer hover:opacity-80"
                            onClick={(e) => {
                              e.stopPropagation();
                    ...
                    {linked && (
                      <div className="text-xs text-sky-600 bg-sky-50 inline-block px-1 rounded mb-1">
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 px-6 py-3 flex justify-around items-center z-30 max-w-md mx-auto">
        <button
          ...
          className={`flex flex-col items-center gap-1 ${
            activeTab === 'plan' ? 'text-sky-600' : 'text-slate-300'
          }`}
        >
          <Icons.Calendar />
...
function AppContent() {
  ...
  useEffect(() => {
    if (loaded) {
      const unsub = Service.subscribe(null, null, (data) =>
        setTrips(data || [])
      );
      return () => unsub && unsub();
    }
